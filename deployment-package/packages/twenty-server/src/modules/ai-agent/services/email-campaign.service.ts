...
export class EmailCampaignService {
  ...
  async executeCampaign(campaignId: string): Promise<CampaignResult> {
    ...
    try {
      ...
      const results = await this.sendBulkEmails(targetLeads, emailTemplate, campaign);
  ...
  private async getTargetLeads(campaign: EmailCampaignWorkspaceEntity): Promise<LeadProspectWorkspaceEntity[]> {
    ...
    query.andWhere('lead.lastContactedCampaignId != :campaignId OR lead.lastContactedCampaignId IS NULL', {
  ...
  private async sendBulkEmails(
    ...
    for (const lead of leads) {
      try {
        ...
        const personalizedEmail = await this.personalizeEmail(emailTemplate, lead);
        ...
        results.errors.push(`Failed to send email to ${lead.email}: ${error.message}`);
  ...
  private generateClaimUrl(companyId: string): string {
    ...
    const baseUrl = process.env.DIRECTORY_BASE_URL || 'https://app.earthcare.network';
    return `${baseUrl}/claim/${companyId}`;
  }

  private generateTrialUrl(): string {
    // Generate URL for Build Pro trial signup
    const baseUrl = process.env.SPATIAL_NETWORK_BASE_URL || 'https://spatial.network';
  ...
  async getCampaignMetrics(campaignId: string): Promise<CampaignMetrics> {
    ...
    const conversionRate = campaign.emailsSent > 0 
